...
// ---------------------------------------------------------------------------
// Serial execution — ordering verification
// ---------------------------------------------------------------------------

test.describe('serial execution — file ordering', () => {
  ...
  test('sequence contains expected test names', () => {
    runPlaywright(SERIAL_CONFIG, outputFile);
    records = readOutput(outputFile);
...
// ---------------------------------------------------------------------------
// Multi-sequence — both sequences run independently
// ---------------------------------------------------------------------------

test.describe('serial execution — multi-sequence', () => {
  test.describe.configure({ mode: 'serial' });
  test.setTimeout(60000);
  ...
  test('checkout-flow order is preserved in multi-sequence run', () => {
    runPlaywright(MULTI_CONFIG, outputFile);
    const records = readOutput(outputFile);
